...
public sealed class ProfilesService : IProfilesService
{
    ...
    public async Task<JsonObject?> LoginAsync(string identifier, string password)
    {
        try
        {
            ...
            var originalProfile = finalProfiles[0]!.AsObject();
            
            // Create a fresh JsonObject to avoid "node already has a parent" errors
            // We'll copy all profile fields and add auth tokens safely
            var responseProfile = new JsonObject();
            
            // Copy all profile fields to the new JsonObject
            foreach (var kvp in originalProfile)
            {
                // Clone the value to avoid parent conflicts
                var clonedValue = JsonNode.Parse(kvp.Value?.ToJsonString() ?? "null");
                responseProfile[kvp.Key] = clonedValue;
            }
            
            // Safely add auth tokens by cloning their values
            if (authResult["access_token"] != null)
            {
                var accessTokenClone = JsonNode.Parse(authResult["access_token"]!.ToJsonString());
                responseProfile["access_token"] = accessTokenClone;
            }
            
            if (authResult["refresh_token"] != null)
            {
                var refreshTokenClone = JsonNode.Parse(authResult["refresh_token"]!.ToJsonString());
                responseProfile["refresh_token"] = refreshTokenClone;
            }
            
            // Add user info from auth response if available
            if (authResult["user"] != null)
            {
                var userClone = JsonNode.Parse(authResult["user"]!.ToJsonString());
                responseProfile["auth_user"] = userClone;
            }
            
            Console.WriteLine($"[PROFILES] Login successful for user: {responseProfile["full_name"]} (role: {responseProfile["role"]})");
            Console.WriteLine($"[PROFILES] Response contains {responseProfile.Count} fields");
